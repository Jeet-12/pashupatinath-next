...
import { createOrder, getUserAddresses, createAddress, getCart } from '../libs/api';
...
const CheckoutPage = () => {
  ...
  const loadAddresses = async () => {
    try {
      ...
      if (addressesResponse.success && addressesResponse.data) {
        ...
        setSelectedAddress(defaultAddress || addressesResponse.data[0] || null);
      }
    } catch (error) {
      console.error('Error loading addresses:', error);
      // Load mock addresses if API fails
  ...
  const total = Math.max(0, subtotal - upiDiscount - couponDiscount + shippingFee + codCharges);
  ...
  const handleSaveAddress = async () => {
    ...
    try {
      ...
      const response = await createAddress(newAddressData);
  ...
  const handleApplyCoupon = () => {
    ...
    if (coupon) {
      ...
      setError('Invalid coupon code. Please try a valid code from the list.');
    }
  };
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        if (selectedPayment === 'razorpay') {
          ...
          await handleRazorpayPayment(razorpayOrderData);
          ...
          <div className="flex justify-center items-center mb-6 md:mb-8 px-2">
            {[1, 2, 3].map((step) => (
              <React.Fragment key={step}>
                <div className={`flex items-center ${activeStep >= step ? 'text-amber-600' : 'text-gray-400'}`}>
                  <div className={`w-8 h-8 md:w-10 md:h-10 rounded-full flex items-center justify-center border-2 text-sm md:text-base ${
                    activeStep >= step ? 'bg-amber-500 border-amber-500 text-white' : 'border-gray-300'
                  }`}>
                    {step}
                  </div>
                  <span className="ml-2 font-semibold text-sm md:text-base hidden sm:block">
                    {step === 1 ? 'Address' : step === 2 ? 'Payment' : 'Confirm'}
                  </span>
                </div>
                {step < 3 && (
                  <div className={`w-12 md:w-24 h-1 mx-2 md:mx-4 ${
                    activeStep > step ? 'bg-amber-500' : 'bg-gray-300'
                  }`}></div>
                )}
              </React.Fragment>
            ))}
                  ...
                  >
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 md:gap-4">
                      <div className="flex items-center space-x-3 md:space-x-4 lg:space-x-6">
                        <div className="w-12 h-12 md:w-14 md:h-14 lg:w-16 lg:h-16 bg-gradient-to-br from-blue-500 to-cyan-600 rounded-lg md:rounded-xl lg:rounded-2xl flex items-center justify-center shadow-lg">
                          <span className="text-white font-bold text-base md:text-lg lg:text-xl">RP</span>
                        </div>
                        <div>
                          <h3 className="text-base md:text-lg lg:text-xl font-bold text-gray-900">Razorpay</h3>
                          <p className="text-gray-600 text-sm md:text-base">Credit/Debit Card, Net Banking, UPI, Wallets</p>
                          <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2 md:gap-4 mt-1 md:mt-2">
                            <span className="bg-blue-100 text-blue-800 text-xs md:text-sm px-2 md:px-3 py-1 rounded-full font-semibold w-fit">
                              Most Secure
                            </span>
                            <span className="text-green-600 font-bold text-sm md:text-base">Instant Payment</span>
                          </div>
                        </div>
                      </div>
                      <div className="flex items-center justify-between sm:justify-end gap-2 md:gap-4">
                        <div className={`w-5 h-5 md:w-6 md:h-6 lg:w-7 lg:h-7 rounded-full border-2 flex items-center justify-center ${
                          selectedPayment === 'razorpay' ? 'border-blue-500 bg-blue-500' : 'border-gray-300'
                        }`}>
                          {selectedPayment === 'razorpay' && (
          ...
          <div className="bg-white rounded-2xl md:rounded-3xl p-4 md:p-6 lg:p-8 max-w-sm md:max-w-md w-full transform hover:scale-105 transition-transform duration-300 mx-2">
            ...
            <div className="space-y-3 md:space-y-4 mb-4 md:mb-6 lg:mb-8">
              <button
                ...
                disabled={isLoading}
                className="w-full bg-gradient-to-r from-green-500 to-emerald-600 text-white py-3 md:py-4 rounded-xl md:rounded-2xl font-bold text-sm md:text-base lg:text-lg hover:from-green-600 hover:to-emerald-700 transition-all duration-300 shadow-lg hover:shadow-xl flex items-center justify-center disabled:opacity-50"
              >
                {isLoading ? (
                  <div className="flex items-center">
