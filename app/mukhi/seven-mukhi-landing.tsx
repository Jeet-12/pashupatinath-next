...
export default function SevenMukhiLandingPage() {
  ...
  // Capping details based on your API response
  const cappingOptions = {
    normal: { id: null, price: 0 },
    silver: { id: 14, price: 200 },
  };
  ...
  const totalPrice = useMemo(() => {
    const variantPrice = cappingOptions[selectedVariant as keyof typeof cappingOptions].price;
    return (currentPrice + variantPrice) * quantity;
  }, [quantity, selectedVariant, currentPrice]);
  ...
  const addToCart = async () => {
    ...
    try {
      ...
      const selected_cap_id = cappingOptions[selectedVariant as keyof typeof cappingOptions].id;
      
      const res = await singleAddToCart({ 
        slug: productDetails.slug, 
        quantity, 
        total_price, 
        selected_cap: selected_cap_id,
      });
            ...
            >
              <div className="bg-gradient-to-br from-amber-100 to-orange-100 rounded-2xl sm:rounded-3xl p-4 sm:p-6 lg:p-8 shadow-2xl">
                ...
                <div className="flex gap-3 sm:gap-4 mt-4 sm:mt-6 justify-center">
                  <div className="text-center cursor-pointer" onClick={() => setSelectedVariant('normal')}>
                    ...
                    } mx-auto flex items-center justify-center transition-all`}>
                      <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-orange-900/80"></div>
                    </div>
                    <span className={`text-xs mt-1 block font-medium ${selectedVariant === 'normal' ? 'text-amber-700' : 'text-gray-600'}`}>Normal</span>
                  </div>
                  <div className="text-center cursor-pointer" onClick={() => setSelectedVariant('silver')}>
                    <div className={`w-12 h-12 sm:w-16 sm:h-16 rounded-lg sm:rounded-xl border-2 ${
                      selectedVariant === 'silver' ? 'border-amber-500 bg-amber-50' : 'border-gray-300 bg-gray-100'
                    } mx-auto flex items-center justify-center transition-all`}>
                      <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-slate-400"></div>
                    </div>
                    <span className={`text-xs mt-1 block font-medium ${selectedVariant === 'silver' ? 'text-amber-700' : 'text-gray-600'}`}>Silver +₹{cappingOptions.silver.price}</span>
                  </div>
                </div>
              </div>
            </motion.div>
            ...
            >
              ...
              >
                ...
                {selectedVariant === 'silver' && (
                  <p className="text-amber-100 text-sm mt-1">Includes Silver Capping (+₹{cappingOptions.silver.price})</p>
                )}
              </motion.div>
              ...
              <div className="space-y-3 sm:space-y-4">
                ...
                <div className="grid grid-cols-2 gap-2 sm:gap-3">
                  ...
                  >
                    ...
                    <div className="text-xs sm:text-sm text-gray-600">+₹{cappingOptions.silver.price} • Enhanced</div>
                  </motion.button>
                </div>
              </div>
